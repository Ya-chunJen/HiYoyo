# Hiyoyo智能音箱
一个基于ChatGPT的智能音箱项目，可以运行在MacOS和Linux上。
...
项目以上项目的不同点在于：
1. 唤醒词方案改为了[snowboy的方案](https://github.com/Kitt-AI/snowboy/blob/master/README_ZH_CN.md)；
2. 支持多个角色并且支持平顺切换；
...
## 安装部署要求
#### 1、硬件要求
- 在M1芯片的Mac上、X86的Linux上、香橙派上均测试成功。
- 目前程序还不支持分别指定输入和输出设备，所以最好配备一个同时支持麦克风和扬声器的设备，并且最好是USB的连接方式而不是蓝牙的连接方式。——如果遇到奇奇怪怪的错误，最好先检查一下音频输入和输出设备。

#### 2、软件要求
- Python3环境。
- 安装pyaudio。
- 使用snowboy项目编译「_ snowboydetect.so」文件时,需要安装比较的软件，请按snowboy项目的说明操作。——这一点可能是最大的障碍点。
...
#### 1、clone本项目到本地
- main.py，程序的主文件
- config.ini，配置文件信息，用于存放需要调用一些接口的key信息
...
- Snowboy 目录下的 Snowboy_Model_Path 为snowboy唤醒词模型的文件路径，如：snowboy/resources/models/snowboy.umdl，这里是相对路径。
- Snowboy 目录下的 Sensitivity 为snowboy唤醒词的灵敏度。
...
- "robot_voice_name":"zh-CN-XiaochenNeural",——智能助手的嗓音名称，嗓音是智能助手最外显的一个特征，使用的是Azure的接口，可用嗓音见[此链接](https://learn.microsoft.com/zh-cn/azure/cognitive-services/speech-service/language-support?tabs=tts) 。
- "robot_reply_word":"嗯，你好！",——唤醒智能助手后的，智能助手打招呼的内容。
- "robot_system_content":"你是一个有用的智能助手"——作为一个有身份的智能助手，这里是请求ChatGPT接口时，定义的system。
